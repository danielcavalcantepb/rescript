...
function KpiCard({ label, value, detail, icon: Icon, href, tone = 'default' }: { label: string; value: string; detail: string; icon: typeof ReceiptText; href: string; tone?: 'default' | 'warning' }) {
  return <Link to={href} className="group relative min-h-40 overflow-hidden rounded-[var(--radius-lg)] border border-[var(--color-border-soft)] bg-[var(--color-surface)] p-4 transition duration-200 hover:-translate-y-0.5 hover:border-[var(--color-primary)]/50 hover:shadow-[var(--shadow-md)]"><div className={cn('absolute -right-6 -top-7 size-24 rounded-full blur-2xl', tone === 'warning' ? 'bg-[var(--color-warning)]/10' : 'bg-[var(--color-primary)]/10')} /><div className="relative flex h-full flex-col"><div className="flex items-center justify-between"><span className="text-xs text-[var(--color-text-secondary)]">{label}</span><Icon className={cn('size-4', tone === 'warning' ? 'text-[var(--color-warning)]' : 'text-[var(--color-primary)]')} /></div><strong className="mt-5 font-mono text-xl font-semibold tracking-tight tabular-nums">{value}</strong><span className="mt-auto text-[11px] text-[var(--color-muted)]">{detail}</span></div></Link>
}
