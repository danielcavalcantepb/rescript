...
import type { SupplierAddressRepository } from '#/modules/suppliers/application/ports'
import type { SupplierAddress } from '#/modules/suppliers/domain/types'
...
import { mapAddress } from '#/modules/suppliers/infrastructure/mappers'
...
type AddressUpdate = Database['public']['Tables']['supplier_address']['Update']

function digitsOnly(value: string): string {
  return value.replace(/\D/g, '') || value.trim()
}

export class SupabaseSupplierAddressRepository
  implements SupplierAddressRepository
{
  ...
  async create(
    organizationId: string,
    userId: string,
    input: Parameters<SupplierAddressRepository['create']>[2],
  ): Promise<SupplierAddress> {
    assertEntityOrganization(organizationId, this.options.organizationId)
    const { data, error } = await this.options.client
      .from('supplier_address')
  ...
  async update(
    ...
    userId: string,
    id: string,
    input: Parameters<SupplierAddressRepository['update']>[3],
  ): Promise<SupplierAddress> {
    assertEntityOrganization(organizationId, this.options.organizationId)
