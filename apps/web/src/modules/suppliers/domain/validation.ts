...
function validateOptionalLengths(
  ...
  if (
    ...
    input.legalName.trim().length > SUPPLIER_LIMITS.legalName
  ) {
    errors.legalName = `Nome deve ter no máximo ${SUPPLIER_LIMITS.legalName} caracteres.`
  }
  if (
    input.tradeName !== undefined &&
    input.tradeName &&
    input.tradeName.trim().length > SUPPLIER_LIMITS.tradeName
  ) {
    errors.tradeName = `Nome fantasia deve ter no máximo ${SUPPLIER_LIMITS.tradeName} caracteres.`
  }
  if (
    input.email !== undefined &&
    input.email &&
    input.email.trim().length > SUPPLIER_LIMITS.email
  ) {
    errors.email = `E-mail deve ter no máximo ${SUPPLIER_LIMITS.email} caracteres.`
  }
  if (
    input.phone !== undefined &&
    input.phone &&
    input.phone.trim().length > SUPPLIER_LIMITS.phone
  ) {
    errors.phone = `Telefone deve ter no máximo ${SUPPLIER_LIMITS.phone} caracteres.`
  }
  if (
    input.city !== undefined &&
    input.city &&
    input.city.trim().length > SUPPLIER_LIMITS.city
  ) {
    errors.city = `Cidade deve ter no máximo ${SUPPLIER_LIMITS.city} caracteres.`
  }
  if (
    input.notes !== undefined &&
    input.notes &&
    input.notes.trim().length > SUPPLIER_LIMITS.notes
  ) {
    errors.notes = `Observações devem ter no máximo ${SUPPLIER_LIMITS.notes} caracteres.`
  }
}

export function validateCreateSupplier(input: CreateSupplierInput): FieldErrors {
  const errors: FieldErrors = {}
  const legalName = input.legalName?.trim() ?? ''
  if (legalName.length < 1) errors.legalName = 'Informe o nome do fornecedor.'
...
export function validateUpdateSupplier(
  ...
  if (input.legalName !== undefined && input.legalName.trim().length < 1) {
    errors.legalName = 'Informe o nome do fornecedor.'
  }
...
export function validateUpdateAddress(input: UpdateAddressInput): FieldErrors {
  ...
  if (
    ...
    !['billing', 'shipping', 'other'].includes(input.kind)
  ) {
    errors.kind = 'Tipo de endereço inválido.'
  }
