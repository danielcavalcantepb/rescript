  ...
  const labels = variant.assignments
    .map((assignment) => {
      const attribute = attributes.find(
        (item) => item.id === assignment.attributeDefinitionId,
      )
...
export function ProductWizard({
  ...
  useEffect(() => {
    if (!draft.unitId && units[0])
      update({ unitId: units[0].id, unitCode: units[0].code })
  }, [units])
  useEffect(() => {
    ...
    setDraft((current) => {
      ...
      const changed =
        ...
        variants.some(
          (variant, index) => variant.branchId !== current.variants[index]?.branchId,
        )
  ...
  useEffect(() => {
    ...
    setDraft((current) => {
      ...
      const changed =
        ...
        variants.some(
          (variant, index) => variant.locationId !== current.variants[index]?.locationId,
        )
      if (!changed) return current
      setDirty(true)
  ...
  const variantErrors = useMemo(
    ...
    [duplicateCombinations, draft.branchId, draft.kind, draft.locationId, variants],
  )
      ...
      <section className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface)]">
        <header className="border-b border-[var(--color-border-soft)] p-5">
          <ol aria-label="Progresso do cadastro" className="grid grid-cols-4 gap-2">
            {['Dados básicos', 'Classificação', 'Variantes', 'Resumo'].map(
              (label, index) => (
                <li key={label} aria-current={step === index + 1 ? 'step' : undefined}>
                  <div
                    className={`h-1 rounded-full ${index + 1 <= step ? 'bg-[var(--color-accent)]' : 'bg-[var(--color-border)]'}`}
                  />
        ...
        <div className="min-h-[470px] p-5 sm:p-7">
          ...
          {step === 2 && <NcmFields draft={draft} update={update} />}
          {step === 2 && (
            <ClassificationStep
              draft={draft}
          ...
          {error && (
            <p
              role="alert"
              className="mt-5 rounded-[var(--radius-md)] border border-[var(--color-danger)]/30 bg-[var(--color-danger)]/10 p-3 text-sm text-[var(--color-danger)]"
            >
              {friendlyError(error)}
            </p>
          )}
          {unitsError && (
            <p role="alert" className="mt-5 text-sm text-[var(--color-danger)]">
              {unitsError}
            </p>
          )}
        </div>
...
function BasicStep({
  ...
  function updateUnit(value: string) {
    ...
    const match = units.find(
      (unit) =>
        unit.code.toLowerCase() === normalized || unit.name.toLowerCase() === normalized,
    )
  ...
  return (
    <div className="space-y-6">
      <Heading
        title="Dados básicos"
        description="Identifique o produto e defina a sua unidade operacional."
      />
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Tipo">
          <select
            className={fieldClass}
            value={draft.kind}
            onChange={(event) => changeKind(event.target.value as ProductKind)}
          >
        ...
        <Field label="Unidade" required>
          <input
            className={fieldClass}
            list="catalog-units"
            value={draft.unitCode || matched?.code || ''}
            onChange={(event) => updateUnit(event.target.value)}
            placeholder="UN, KG, MT, CX…"
          />
          <datalist id="catalog-units">
            {units.map((unit) => (
              <option key={unit.id} value={unit.code} label={unit.name} />
            ))}
          </datalist>
          <p className="text-xs font-normal text-[var(--color-ink-muted)]">
            {matched ? `${matched.name} selecionada.` : 'Digite uma unidade cadastrada.'}
          </p>
        </Field>
      </div>
...
function NcmFields({
  ...
  return (
    <>
      <div className="mb-5 rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-4">
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <Field label="Descrição NCM">
            <div className="flex gap-2">
              <input
                className={fieldClass}
                value={draft.ncmDescription}
                onChange={(event) => update({ ncmDescription: event.target.value })}
        ...
        <p className="mt-2 text-xs text-[var(--color-ink-muted)]">
          Preencha manualmente, gere o padrão ou consulte a tabela oficial sem sair do
          cadastro.
        </p>
      </div>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent
          size="lg"
          aria-describedby="ncm-search-description"
          className="max-h-[85vh] overflow-y-auto"
        >
          <DialogTitle>Seleção NCM de Produtos e Serviços</DialogTitle>
          <DialogDescription id="ncm-search-description">
            Pesquise pelo código ou descrição e selecione a classificação correta.
          </DialogDescription>
          <form
          ...
          >
            <input
              ...
              placeholder="Ex.: 6109 ou camiseta"
              autoFocus
            />
          ...
          <div className="mt-5 overflow-x-auto rounded-[var(--radius-md)] border border-[var(--color-border)]">
            <table className="w-full min-w-[540px] text-left text-sm">
              <thead className="bg-[var(--color-surface-subtle)] text-[var(--color-ink-muted)]">
                <tr>
                  <th className="px-3 py-2 font-medium">Código NCM</th>
                  <th className="px-3 py-2 font-medium">Descrição</th>
                  <th className="w-24 px-3 py-2 font-medium">Ação</th>
                </tr>
              </thead>
              <tbody>
                {results.map((item) => (
                  <tr key={item.code} className="border-t border-[var(--color-border)]">
                    <td className="px-3 py-3 font-mono">{item.code}</td>
                    <td className="px-3 py-3">{item.description}</td>
                    <td className="px-3 py-3">
                      <Button type="button" size="sm" onClick={() => select(item)}>
                        Usar
                      </Button>
                    </td>
                  </tr>
  ...
  return (
    <div className="space-y-7">
      <div>
        <Heading
          title="Classificação"
          description="Defina categoria, marca e os padrões que serão usados ao iniciar as variantes."
        />
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <SelectField
            ...
            items={activeCategories.map((item) => ({ id: item.id, label: item.name }))}
            emptyText="Sem categorias ativas"
            configTo="/catalog/categories"
          />
          <SelectField
            label="Marca"
            value={draft.brandId}
            onChange={(brandId) => update({ brandId })}
            items={activeBrands.map((item) => ({ id: item.id, label: item.name }))}
            emptyText="Sem marcas ativas"
            configTo="/catalog/brands"
          />
          <SelectField
            label="Lista de preço"
            required
            value={draft.priceListId}
            onChange={(priceListId) => update({ priceListId })}
            items={activeLists.map((item) => ({
              id: item.id,
              label: `${item.name}${item.isDefault ? ' · padrão' : ''}`,
            }))}
            emptyText="Nenhuma lista de preço ativa"
            configTo="/catalog/pricing"
          />
          <SelectField
            label="Filial padrão"
            required
            ...
            emptyText="Nenhuma filial ativa"
          />
          <SelectField
            label="Localização padrão"
            value={draft.locationId}
            onChange={(locationId) => update({ locationId })}
            items={activeLocations.map((item) => ({ id: item.id, label: item.name }))}
            emptyText="Nenhuma localização ativa"
            configTo="/catalog/inventory"
          />
        </div>
      </div>
      <div className="rounded-[var(--radius-md)] border border-dashed border-[var(--color-border)] p-4 text-sm text-[var(--color-ink-muted)]">
        {draft.kind === 'simple'
          ? 'Será criada uma única variante padrão. Atributos não são necessários.'
          : 'Na próxima etapa, crie cada variante e informe somente os atributos que formam aquela combinação.'}
      </div>
    </div>
  )
}
...
function VariantsStep({
  ...
  updateVariant: (id: string, next: Partial<VariantDraft>) => void
  addVariant: (copy?: VariantDraft) => void
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <Heading
          title={draft.kind === 'simple' ? 'Variante padrão' : 'Variantes'}
          description={
            draft.kind === 'simple'
              ? 'Preencha os dados operacionais da única variante do produto.'
              : 'Comece por uma variante. Os atributos abaixo identificam apenas esta combinação.'
          }
        />
        <Button
          type="button"
          onClick={() => addVariant()}
          disabled={draft.kind === 'simple'}
        >
          + Adicionar variante
        </Button>
      </div>
      <div className="space-y-4">
        {variants.map((variant, index) => (
          <VariantCard
            ...
            branches={branches}
            locations={locations}
            defaultBranch={draft.branchId}
            defaultLocation={draft.locationId}
            error={errors.find((item) => item.startsWith(`${variant.id}:`))}
            canRemove={draft.kind === 'variable' && variants.length > 1}
            removing={removing === variant.id}
            update={updateVariant}
            onDuplicate={() => addVariant(variant)}
...
function VariantCard({
  ...
  canRemove: boolean
  removing: boolean
  update: (id: string, next: Partial<VariantDraft>) => void
  onDuplicate: () => void
  onRemove: () => void
}) {
  const availableAttributes = attributes.filter((attribute) =>
    attribute.values.some((value) => value.status === 'active'),
  )
  const assigned = new Set(
    variant.assignments.map((assignment) => assignment.attributeDefinitionId),
  )
  function setAssignment(attributeDefinitionId: string, optionId: string) {
    const remaining = variant.assignments.filter(
      (assignment) => assignment.attributeDefinitionId !== attributeDefinitionId,
    )
    update(variant.id, {
      assignments: optionId
        ? [...remaining, { attributeDefinitionId, optionId }]
        : remaining,
    })
  }
  return (
    <article className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-4">
        ...
        <div className="flex gap-2">
          ...
          {canRemove && (
            <Button type="button" size="sm" variant="secondary" onClick={onRemove}>
              {removing ? 'Confirmar remoção' : 'Remover'}
            </Button>
          )}
        </div>
      </div>
      {availableAttributes.length > 0 && (
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <Field label="Adicionar atributo">
            <select
              className={fieldClass}
              value=""
              onChange={(event) => {
                ...
                if (attribute && option) setAssignment(attribute.id, option.id)
              }}
            >
              <option value="">Selecione um atributo</option>
              {availableAttributes
                .filter((attribute) => !assigned.has(attribute.id))
                .map((attribute) => (
            ...
            return (
              <div key={attribute.id} className="space-y-1.5">
                <div className="flex items-center justify-between gap-2">
                  <label className="text-sm font-medium" htmlFor={`${variant.id}-${attribute.id}`}>
                    {attribute.name}
                  </label>
                  <button
                    type="button"
                    className="text-xs font-medium text-[var(--color-danger)] underline-offset-2 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-focus)]"
                    onClick={() => setAssignment(attribute.id, '')}
                    aria-label={`Remover atributo ${attribute.name} da variante ${index + 1}`}
                  >
                    Remover
                  </button>
                </div>
                <select
                  ...
                  onChange={(event) => setAssignment(attribute.id, event.target.value)}
                >
                  <option value="">Remover atributo</option>
                  {attribute.values
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        ...
        <Field label="EAN">
          <input
            ...
            value={variant.ean}
            onChange={(event) =>
              update(variant.id, { ean: event.target.value.replace(/\D/g, '') })
            }
            inputMode="numeric"
          />
        </Field>
        <Field label="Preço de venda" required>
          <input
            ...
            onChange={(event) => update(variant.id, { salePrice: event.target.value })}
            inputMode="decimal"
            placeholder="0,00"
          />
        </Field>
        ...
        <Field label="Margem">
          <output
            className={`${fieldClass} flex items-center bg-[var(--color-surface-subtle)] text-[var(--color-ink-muted)]`}
          >
            {derivedMargin(variant.salePrice, variant.unitCost) == null
              ? variant.salePrice
                ? 'Sem custo'
                : 'Sem base'
              : `${derivedMargin(variant.salePrice, variant.unitCost)?.toLocaleString('pt-BR')}%`}
          </output>
        </Field>
        <Field label="Quantidade inicial">
          <input
            ...
            onChange={(event) =>
              update(variant.id, { quantity: event.target.value.replace(/[^0-9]/g, '') })
            }
            inputMode="numeric"
          />
        </Field>
        <Field label="Filial">
          <select
            className={fieldClass}
            value={variant.branchId || defaultBranch}
            onChange={(event) => update(variant.id, { branchId: event.target.value })}
          >
        ...
        <Field label="Localização">
          <select
            className={fieldClass}
            value={variant.locationId || defaultLocation}
            onChange={(event) => update(variant.id, { locationId: event.target.value })}
          >
        ...
        <Field label="Status">
          <select
            ...
            onChange={(event) =>
              update(variant.id, { status: event.target.value as 'active' | 'inactive' })
            }
          >
...
function ReviewStep({
  ...
  return (
    <div className="space-y-6">
      <Heading
        title="Revisão final"
        description="Revise todos os dados antes de iniciar a transação de criação."
      />
      <div className="grid gap-3 sm:grid-cols-2">
        <Review label="Produto" value={draft.name} onEdit={() => onStep(1)} />
        <Review
          label="Classificação"
          value={`${categories.find((item) => item.id === draft.categoryId)?.name ?? 'Sem categoria'} · ${brands.find((item) => item.id === draft.brandId)?.name ?? 'Sem marca'}`}
          onEdit={() => onStep(2)}
        />
        <Review
          label="Lista de preço"
          value={selectedPriceList?.name ?? 'Não selecionada'}
          onEdit={() => onStep(2)}
        />
        <Review
          label="Variantes"
          value={`${variants.length} cadastrada(s)`}
          onEdit={() => onStep(3)}
        />
      </div>
      {!selectedPriceList && (
        <div
          role="alert"
          className="rounded-[var(--radius-md)] border border-[var(--color-danger)]/30 p-4 text-sm text-[var(--color-danger)]"
        >
          Uma lista de preço ativa é obrigatória para criar o produto.{' '}
          <button
            type="button"
          ...
          ou{' '}
          <Link className="font-semibold underline" to="/catalog/pricing">
            criar uma lista
          </Link>
          .
        </div>
      ...
      <div className="overflow-x-auto rounded-[var(--radius-md)] border border-[var(--color-border-soft)]">
        <table className="min-w-full text-left text-sm">
          <thead className="border-b border-[var(--color-border-soft)] text-xs text-[var(--color-ink-muted)]">
            <tr>
              <th className="p-3">Combinação</th>
          ...
          <tbody>
            {variants.map((variant) => (
              ...
              >
                <td className="p-3">{variantLabel(variant, attributes)}</td>
                <td className="p-3">{variant.sku || '—'}</td>
                <td className="p-3 font-mono">{variant.ean || '—'}</td>
                <td className="p-3">{formatMoney(variant.unitCost)}</td>
                <td className="p-3">{formatMoney(variant.salePrice)}</td>
                <td className="p-3">
                  {derivedMargin(variant.salePrice, variant.unitCost) == null
                    ? 'Sem base'
                    : `${derivedMargin(variant.salePrice, variant.unitCost)}%`}
                </td>
                <td className="p-3">{variant.quantity || '0'}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {errors.length > 0 && (
        ...
        >
          ...
          <ul className="mt-2 list-disc pl-5">
            {errors.map((item) => (
              <li key={item}>{item.slice(item.indexOf(':') + 1)}</li>
            ))}
          </ul>
        </div>
      )}
  ...
  return (
    <aside className="h-fit rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-5 xl:sticky xl:top-5">
      ...
      <dl className="mt-5 space-y-3 text-sm">
        <Row label="Tipo" value={draft.kind === 'simple' ? 'Simples' : 'Variável'} />
        <Row
          label="Categoria"
          value={
        ...
        <Row
          label="Marca"
          value={brands.find((item) => item.id === draft.brandId)?.name ?? 'Não definida'}
        />
        <Row label="Atributos em uso" value={String(selectedAttributes)} />
        <Row label="Variantes cadastradas" value={String(variants.length)} />
        <Row label="Status" value={stepStatus(draft)} />
      </dl>
      <p className="mt-5 text-xs text-[var(--color-ink-muted)]">
        Cada variante concentra sua combinação, preço, custo e quantidade inicial.
      </p>
    </aside>
  )
}

function stepStatus(draft: Draft) {
  return draft.name.trim() && draft.unitId
    ? 'Em preenchimento'
    : 'Dados básicos pendentes'
}
function validateVariant(
  ...
  const errors: string[] = []
  const quantity = numberValue(variant.quantity) ?? 0
  if (!variant.sku.trim()) errors.push('SKU é obrigatório.')
  if (
    numberValue(variant.salePrice) == null ||
    (numberValue(variant.salePrice) ?? -1) < 0
  )
    errors.push('Informe um preço de venda válido.')
  if (numberValue(variant.unitCost) != null && (numberValue(variant.unitCost) ?? 0) < 0)
    errors.push('Custo unitário não pode ser negativo.')
  if (!Number.isInteger(quantity) || quantity < 0)
    errors.push('Quantidade deve ser um inteiro não negativo.')
  if (
    quantity > 0 &&
    (!(variant.locationId || defaultLocationId) || !(variant.branchId || defaultBranchId))
  )
    errors.push('Selecione filial e localização para o saldo inicial.')
  if (kind === 'variable' && variant.assignments.length === 0)
    errors.push('Selecione pelo menos um atributo.')
  if (duplicate) errors.push('Combinação de atributos repetida.')
  return errors.map((message) => `${variant.id}:${message}`)
}
function friendlyError(error: string) {
  const map: Record<string, string> = {
    variant_sku_required: 'Informe o SKU de cada variante.',
    initial_price_required: 'Informe um preço de venda válido.',
    active_price_list_required: 'Selecione uma lista de preço ativa.',
    valid_branch_required: 'Selecione uma filial válida.',
    valid_stock_location_required:
      'Selecione uma localização válida para o estoque inicial.',
    initial_quantity_invalid: 'A quantidade inicial é inválida.',
    initial_unit_cost_invalid: 'O custo inicial é inválido.',
    category_not_found: 'A categoria informada não é válida.',
    brand_not_found: 'A marca informada não é válida.',
    idempotency_key_payload_mismatch:
      'Esta tentativa já foi enviada com dados diferentes. Atualize a página para iniciar uma nova tentativa.',
  }
  return Object.entries(map).find(([key]) => error.includes(key))?.[1] ?? error
}
...
function Field({
  ...
  const eanInput =
    label === 'EAN' &&
    isValidElement<{ onChange?: (event: React.ChangeEvent<HTMLInputElement>) => void }>(
      children,
    )
      ? children
      : null
  ...
  return (
    <label className="block space-y-1.5 text-sm font-medium">
      ...
      {eanInput ? (
        <p className="text-xs font-normal text-[var(--color-ink-muted)]">
          Digite manualmente ou gere um EAN-13.
        </p>
      ) : null}
    </label>
  )
...
function Review({
  ...
  return (
    <div className="rounded-[var(--radius-md)] border border-[var(--color-border-soft)] p-3">
      <dt className="text-xs text-[var(--color-ink-muted)]">{label}</dt>
      <dd className="mt-1 flex items-start justify-between gap-2 font-medium">
        <span>{value}</span>
        <button
          type="button"
  ...
  return (
    <Field label={label} required={required}>
      {items.length ? (
        ...
        <div className="rounded-[var(--radius-md)] border border-dashed border-[var(--color-border)] p-3 text-sm text-[var(--color-ink-muted)]">
          {emptyText}
          {configTo && (
            <>
              {' '}
