...
| Catalog / Products / Variants | Implemented | Partial enterprise | Overview, Products, Categories, Brands and minimum Attributes Workspace implemented | Canonical Product, Variant, Category, Brand, Attribute and Attribute Value server commands | Foundation tables plus structured Variant projection implemented | Append-only taxonomy/assignment history plus Product lifecycle history | Canonical `catalog.*` keys with legacy compatibility | Unit, persistence, RLS and projection coverage | Complete editor depth, advanced filters, images, fiscal context and route cutover |
