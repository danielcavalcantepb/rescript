...
| Command Center | Partial | Foundation | Implemented resilient read-only Workspace at `/app` | Server function `getCommandCenter` through application/read port; source timeout degrades to explicit data gaps | Uses existing operational projections; no canonical aggregate projection yet | No persisted audit by design in read-only foundation | `insights.view` enforced in UI and server | Domain, application, Workspace, and stalled-source resilience tests | Add scalable aggregated projection, RPC/cross-tenant tests against Supabase, and richer fulfillment-aware operational priorities |
...
| Purchasing / Purchase Orders | Implemented | Functional | Orders and server-side query Workspace; creation/detail operational | Server functions over application commands | Aggregate, items, immutable snapshots, canonical lifecycle and item-aware search projection | Append-only history for create/update/send/confirm/cancel/close | Canonical `purchasing.orders.*` in UI/application/PostgreSQL with legacy aliases | Unit, integration, lifecycle, projection, RLS and tenant coverage | Replace raw variant identifier with Catalog picker and move creation into the consolidated drawer |
